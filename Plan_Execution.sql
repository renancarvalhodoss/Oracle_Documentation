...
execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_250337', task_owner => 'SYS', replace => TRUE);



 execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_250334', task_owner => 'SYS', replace => TRUE);
execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_250333', task_owner => 'SYS', replace => TRUE);

execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_250332', task_owner => 'SYS', replace => TRUE);


 execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_248816',task_owner => 'SYS', replace => TRUE);

execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_248816', task_owner => 'SYS', replace => TRUE, profile_type => DBMS_SQLTUNE.PX_PROFILE);


    execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_986879', task_owner => 'SYS', replace => TRUE);

 execute dbms_stats.gather_table_stats(ownname => 'FDSPPRD', tabname =>'PS_DSP_REM01_TMP5', estimate_percent => DBMS_STATS.AUTO_SAMPLE_SIZE, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);

 execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_245581', task_owner => 'SYS', replace => TRUE);


execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_248489', task_owner => 'SYS', replace => TRUE);
)yBH(L0&iFKM1eY)1*

    execute dbms_sqltune.accept_sql_profile(task_name => 'TASK_248504', task_owner => 'SYS', replace => TRUE);
